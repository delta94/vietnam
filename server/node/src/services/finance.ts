...
import { ssi, tanViet, utils, taskQueueProcessor } from './../libs';
import {
...
export default class FinanceService {
  ...
  public async getTopRealTimeStock(): Promise<any> {
    const markets = ['VN30', 'HNX30'];
    let data = [];
    for (const market of markets) {
      let _data: any = await ssi.getRealTimeStockByGroup(market);
  ...
  public async getBasicBEbyCompanies(companies: Array<any>, options: any): Promise<any> {
    const self = this;
    const key = 'P/E cơ bản';
    return await self.getFinancialIndicatorByCompanies(key, companies, options);
  }

  public async getDilutedPEbyCompanies(companies: Array<any>, options: any): Promise<any> {
    const self = this;
    const key = 'P/E pha loãng';
    return await self.getFinancialIndicatorByCompanies(key, companies, options);
  }

  public async getPBbyCompanies(companies: Array<any>, options: any): Promise<any> {
    const self = this;
    const key = 'Hệ số Giá Cổ phiếu/Trị giá Sổ sách (P/B)';
    return await self.getFinancialIndicatorByCompanies(key, companies, options);
  }

  public async getPSbyCompanies(companies: Array<any>, options: any): Promise<any> {
    const self = this;
